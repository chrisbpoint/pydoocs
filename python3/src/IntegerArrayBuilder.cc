...
PyObject* IntegerArrayBuilder::read(EqAdr*, EqData*, EqData* data_from_doocs, PyObject* doocs_parameters) const {
    if (!doocs_parameters) {
        return integer_array_from(data_from_doocs);
    } else {
        throw PyDoocsException::functionality_not_supported_yet();
    }
}

PyObject* IntegerArrayBuilder::integer_array_from(EqData* data_from_doocs) const {
    std::vector<int> array_data;
    for (int i = 0; i < data_from_doocs->array_length(); ++i) {
        array_data.push_back(data_from_doocs->get_int(i));
    }

    PyObject* numpy_array = build_numpy_array(array_data, NPY_INT);
...
PyObject* IntegerArrayBuilder::write(EqAdr* address, EqData* data_to_doocs, EqData* data_from_doocs,
    ...
    if (!doocs_parameters) {
        ...
        for (std::int64_t i = 0; i < PyList_Size(data_from_python); ++i) {
            array_data.push_back(static_cast<int>(PyLong_AsLong(PyList_GetItem(data_from_python, i))));
        }
