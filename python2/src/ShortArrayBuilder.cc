...
PyObject* ShortArrayBuilder::read(EqAdr*, EqData*, EqData* data_from_doocs, PyObject* doocs_parameters) const {
    if (!doocs_parameters) {
        return short_array_from(data_from_doocs);
    } else {
...
PyObject* ShortArrayBuilder::short_array_from(EqData* data_from_doocs) const {
    ...
    PyObject* numpy_array = build_numpy_array(array_data, NPY_SHORT);
...
PyObject* ShortArrayBuilder::write(EqAdr* address, EqData* data_to_doocs, EqData* data_from_doocs,
