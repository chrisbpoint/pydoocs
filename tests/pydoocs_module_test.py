...
class PydoocsModuleTest(unittest.TestCase):
    ...
    def test_the_existence_and_correct_value_of_the_name_attribute(self):
        assert_that("__name__", is_in(dir(self._pydoocs)))
        assert_that(self._pydoocs.__name__, is_(equal_to("pydoocs")))
    ...
    def test_that_all_pydoocs_members_have_the_specified_doc_strings(self):
        assert_that(self._pydoocs.__doc__,
                    is_(equal_to("Python bindings to the standard DOOCS client API written in C/C++.")))
        ...
        assert_that(self._pydoocs.names.__doc__,
                    is_(equal_to("Function to query names from a given DOOCS name domain.")))
        assert_that(self._pydoocs.DoocsException.__doc__,
                    is_(equal_to("Custom DOOCS exception with dict_keys(['code', 'message']) in its first argument.")))
        assert_that(self._pydoocs.PyDoocsException.__doc__,
                    is_(equal_to("Custom pydoocs exception with specific message.")))
